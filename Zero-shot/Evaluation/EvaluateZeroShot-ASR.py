...
for allowed_model in ["small","medium","large-v2"]:

    print("="*50)
    print(allowed_model)

    for file_name in os.listdir("./results_zeroshot/"):
        ...
        f = open(f"./results_zeroshot/{file_name}")
        data = json.load(f)
        f.close()
    ...
    for model_name in sorted(list(models_results.keys())):
        ...
        line = f"{model_name} & " + " & ".join(formatted_values_out) + " \\\\"
        print(line)
