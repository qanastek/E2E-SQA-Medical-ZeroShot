...
# python PredictZeroShotPipeline.py --batch_size=16 --model_name="laion/clap-htsat-unfused"
# python PredictZeroShotPipeline.py --batch_size=16 --model_name="laion/larger_clap_general"
...
for corpus_name in bad_corpus_names:
    ...
    for batch in tqdm(batches):
        ...
        batch_res = audio_classifier(batch_paths, candidate_labels=[f"Option {letter}" for letter in ["A","B","C","D"]])

        for res_preds, res_labels in zip(batch_res, batch_labels):

            scores = [r["score"] for r in res_preds]
            labels = [r["label"][-1] for r in res_preds]
...
with open(f"./results_zero_shot/{full_name}.json", 'w') as f:
