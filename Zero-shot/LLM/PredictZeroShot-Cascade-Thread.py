...
# python PredictZeroShot-Cascade-Thread.py --model_name="microsoft/phi-1_5"
...
# all_corpus = ["MMLU_" + subject for subject in ["clinical_knowledge","medical_genetics"]]
...
def process(data):
    ...
    for current_data in tqdm(data):
        ...
        results.append({
            ...
            "og_identifier": current_data["og_identifier"],
...
for corpus in all_corpus:
    ...
    for size_whisper in ["small","medium","large-v2"]:
        ...
        with torch.no_grad():
            ...
            for d in tqdm(dataset):
                ...
                inputs = tokenizer(re.sub("\s+", " ", d[size_whisper]), return_tensors = "pt")
                ...
                data_threads.append({
                    ...
                    "identifier": d["ID"],
                    "og_identifier": d["ID_original_dataset"],
                    "correct_letter": d["class"].upper(),
                    "classes": ["A","B","C","D"]
                })
        ...
        f_out = open(f"./results_zeroshot/results_{short_model_name}_ZeroShot_{corpus}_EN_{size_whisper}.json", 'w')
